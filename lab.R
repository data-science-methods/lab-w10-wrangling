...
ri_df <- read_excel(file.path(data_folder, '7495_Robust-Intelligence.xlsx'),n_max=25)
...
#' A: For unique project identifiers, we need `AwardNumber` and `Title`; to ensure whether projects are cognitive science-related, we need `Program(s)`; to ensure projects are conducted at UC campuses, we need `Organization`; to find out the amount of money, we need `AwardedAmountToDate`; to find out the year when the project started, we need `StartDate`
#' 
#' 
...
#' A: Most of the first 8 awards are correct, but for 1652633 and 1651909, the number on the website does not match with the number in the dataset
#' 
#' 
...
#' A: `StartDate` is a character variable, some dates are formatted in mm/dd/yyyy and some are more like direct number of dates
...
int_dates1 <- as.Date(as.integer(dates), origin = '1899-12-30')
...
#' A: It confused the order of month and day, while the correct form should be YYYY-MM-DD, our variable is instead displaying YYYY-DD-MM
#' 
#' 
...
parsed_dates <- as.Date(ifelse(is.na(mdy_dates)==T,
                               int_dates2,
...
parse_dates = function(dates) {
     ...
     parsed_dates = as.Date(ifelse(is.na(mdy_dates)==T,int_dates,mdy_dates),
                            origin='1970-01-01')
...
read_and_parse <- function(path){
  ...
  data$StartDate=parse_dates(data$StartDate)
  return(data)
}
