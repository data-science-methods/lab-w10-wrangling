...
ri_df = readxl::read_excel(file.path(data_folder, '7495_Robust-Intelligence.xlsx'),
                           n_max = 25)
#' 
...
#' 'Organization', 'StartDate', and 'AwardedAmountToDate' seem to be important variables to look at
#' 
...
#' 'AwardedAmountToDate' does seem to contain valid and correctly-formatted data. When comparing, all but the 6th and 7th entries had matching numbers. The 6th entry wasn't too far off (16,000 greater on website) but the 7th entry was quite different (229,131 greater on website).
#' 
...
#' The column 'StartDate' contains characters.
#' It seems like some are formatted as MM/DD/YYYY while others just look like a string of numbers (ex: 43109 for the first entry)
...
#' For award 1813043, 'int_dates1' represented the start date as January 9, 2018, rather than September 1, 2018, so the month and day numbers have switched positions.
#' Not exactly sure why Excel failed to parse dates with MM/DD/YYYY format. It may have to do with the fact that those cells are in "General" format rather than "Number" like the other cells in the column so that there would be a zero in front of the month (leading zero).
#' 
...
read_and_parse = function(path) {
  df = read_excel(path)
  df$StartDate = parse_dates(df$StartDate)
...
#' Assertion for all 'StartDate' values being greater than or equal to January 1, 1999
assert_that(all(cleaned_df$StartDate >= '1999-01-01'))
#'
#' Assertion for all of the values being less than or equal to December 31, 2020
